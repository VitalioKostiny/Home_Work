...
class DataValidator:
    ...
    def validate_data(data) -> list[dict[str, Any]]:
        """
                ...
                :return: список валидных данных
                """
        cities_valid = []
        for item in data:
            cities_valid_temp = {}
            var1 = item['subject'].replace(' ', '', 2).isalpha()
            var2 = item['subject'].replace('-', '', 2).isalpha()
            var3 = item['subject'].replace(' ', '', 2).replace('-', '', 2).isalpha()
            if var1 or var2 or var3:
                cities_valid_temp['subject'] = item['subject']
            if item['name'].replace(' ', '', 2).isalpha() or item['name'].replace('-', '', 2).isalpha():
                cities_valid_temp['name'] = item['name']
            if str(item['population']).isdigit():
                cities_valid_temp['population'] = item['population']
            if len(cities_valid_temp) == 3:
